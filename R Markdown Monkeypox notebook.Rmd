...
Write to a file so no need to continuously access from NCBI
```{r}
make.blast.db <- function (sequences, filename){
  db <- filename
  cat("", file = db)
...
```{r}
make.blast.db(ncbi.seq, "C:/Users/samma/Monkeypox-genetics/data/DNAdb.fsa")
```

## Writing to a file - Sam Edition
Alternatively, write to .csv file since I can't get Danny's file to read back in nicely. 
```{r}
...
seq <- unlist(list(db.test["seq"]))       # convert to list and unlist to return a vector
names <- unlist(list(db.test["names"]))   # convert to list and unlist to return a vector
names(seq) <- names                       # assign names to sequences
